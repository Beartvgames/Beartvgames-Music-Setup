...
module.exports = {
  ...
  options: [{
    name: "numero",
    description: "Escribe cuántas canciones quieres omitir.",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (number) {
        ...
        return interaction.reply({ embeds: [vacio], ephemeral: true }).catch(e => { })
      }
      } else {
        try {
          let old = queue.songs[0];
          const success = await queue.skip();
            let skipeed = new EmbedBuilder()
            .setTitle("Skipped")
            .setDescription(`**${old.name}**, ${lang.msg83}`)
            .setColor("#18de29")
            
             let errorskiped = new EmbedBuilder()
            .setTitle("Error :x:")
            .setDescription(`${lang.msg41}`)
            .setColor("#de2c18")
            
          return interaction.reply({ embeds: success ? [skipeed] : [errorskiped] }).catch(e => { })
        } catch (e) {
          return interaction.reply({ embeds: [vacio], ephemeral: true }).catch(e => { })
        }
      }
